...
const makeFormValidatorFactory = <T extends string>(validators: Record<T, (value: string) => string | void>) => <TObject extends string>(config: Record<TObject, Array<T>>) => {
  return (values: Record<TObject, string>) => {
    const errors = {} as Record<TObject, string | undefined>;
